"""
Serverless Executor Worker
Background worker that executes agents via RunAgent Serverless SDK
"""
import asyncio
import logging
import time
...
logger = logging.getLogger("runagent_pulse.serverless_executor")


class ServerlessExecutorWorker:
    """Background worker that polls for and executes serverless agent tasks"""
    ...
    def __init__(
        ...
        self.worker_id = f"serverless-executor-{uuid.uuid4().hex[:8]}"
    ...
    async def start(self):
        ...
        logger.info(f"ServerlessExecutorWorker started (worker_id={self.worker_id})")
    ...
    async def stop(self):
        ...
        try:
            ...
            pass
        self._task = None
        logger.info("ServerlessExecutorWorker stopped")
    ...
    async def _run(self):
        ...
        while True:
            try:
                ...
                logger.error(f"Error in serverless executor worker: {exc}", exc_info=True)
    ...
    async def _execute_task(
        ...
        """Execute a single task by calling RunAgent Serverless"""
        ...
        try:
            ...
            # Import RunAgentClient (will be installed in container)
            try:
                from runagent import RunAgentClient
            except ImportError:
                logger.error("runagent package not installed. Install with: pip install runagent")
                raise
            ...
            client = RunAgentClient(
                ...
                local=False,  # Always use serverless
                user_id=user_id,
